...
class Settings(BaseModel):
    ...
    cache_dir: Path = Path("/kaggle/working/.hf_cache")

    # generation
    max_new_tokens: int = 2048
    temperature: float = 0.2          # coding => low
    top_p: float = 0.8
    top_k: int = 20
    ...
    fs_allowed_roots: tuple[str, ...] = (str(workspace), str(sandbox))
...
os.environ["TRANSFORMERS_CACHE"] = str(settings.cache_dir)
